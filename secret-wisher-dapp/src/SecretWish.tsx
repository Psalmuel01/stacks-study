  ...
  const submitWish = async () => {
    ...
    try {
      ...
      const response = await callContract({
        ...
        functionArgs: [Cl.stringUtf8(wish.text)],
      });
          ...
          {isLoading && wishes.length === 0 ? (
            ...
            <div className="text-center py-12">
              <p className="text-white/70 text-xl">
                No wishes yet. Be the first to make a secret wish!
              </p>
            </div>
          ) : (
            wishes.map((wish) => (
              <div
                key={wish.id}
                className="bg-white/15 backdrop-blur-lg rounded-3xl p-8 border border-white/20
